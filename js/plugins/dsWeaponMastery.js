...
(function(ns) {
	...
	Game_Actor.prototype.wml = function(item)
	{
		if ( DataManager.isSkill(item) )
		{
			...
			else
			{
				if ( item.meta.wmType )
				{
					wmLevelMax = this.wmLevel(Number(item.meta.wmType));
				}
			}
			return wmLevelMax;
		}
	...
	{
		var wmExpMax = ns.Param.MasteryLevelMax * ns.Param.MasteryExpMax;
		var idx = Utility.wmIndex(wtypeId);
		if ( idx >= 0 )
		{
	...
	Game_Actor.prototype.gainWMExp = function(show)
	{
		...
		for ( var ii = 0; ii < ns.Param.MasteriesMax; ii++ )
		{
			...
			while ( this._wmExp[ii] >= (this._wmLevel[ii] + 1) * ns.Param.MasteryExpMax )
			{
			...
			while ( this._wmExp[ii] < this._wmLevel[ii] * ns.Param.MasteryExpMax )
			{
